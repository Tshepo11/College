...
<div class="lefttop1">
  <div class="lefttopleft"> <a href="img/ri.png" rel="facebox"><img src="img/rich.png" width="150" height="60" /></a></div>
   <div class="propic">
...
$received = mysql_query("SELECT * FROM messages WHERE recipient = '$member_id'")or die(mysql_error());
								...
								echo '<font color="Red">'  .strtoupper($receiveda) .'</font>';
...
<div class="right">
	...
	<div class="bkb"></div>
			...
			<?php
						...
						$id = $_SESSION['SESS_MEMBER_ID'];
						$post = mysql_query("SELECT * FROM members WHERE member_id != '$id' AND member_id NOT IN (SELECT friends_with FROM friends WHERE member_id = '$id' ) ORDER BY firstname ASC ")or die(mysql_error());
						while($row = mysql_fetch_array($post)){
							echo '
							<ul id="sddm11">
							<li> 
								<a href="friendprofile.php?id='.$row['member_id'].'"><img class="img" src="'.$row['profImage'].'" alt="" height="40" width="40" " />
								<font color="#1d3162">'.strtoupper($row['FirstName'])." ".strtoupper($row['LastName']).'</font>
								</br>
...
while($row = mysql_fetch_array($result))
  ...
  echo "</br>";
  echo "Contact No: "."".$row['ContactNo']. " | " ."Email: ".$row['email'];
  echo "</br>";
